...
const adminDeleteGroupMember = `-- name: AdminDeleteGroupMember :one
...
type AdminDeleteGroupMemberParams struct {
...
func (q *Queries) AdminDeleteGroupMember(ctx context.Context, arg AdminDeleteGroupMemberParams) (sql.NullInt64, error) {
	row := q.db.QueryRowContext(ctx, adminDeleteGroupMember, arg.GroupID, arg.UserID, arg.UserID_2)
...
const userLeaveGroup = `-- name: UserLeaveGroup :one
DELETE FROM group_memberships
WHERE group_id = $1 AND user_id = $2
RETURNING user_id
`

type UserLeaveGroupParams struct {
	GroupID sql.NullInt64
	UserID  sql.NullInt64
}

// User can leave a group
func (q *Queries) UserLeaveGroup(ctx context.Context, arg UserLeaveGroupParams) (sql.NullInt64, error) {
	row := q.db.QueryRowContext(ctx, userLeaveGroup, arg.GroupID, arg.UserID)
	var user_id sql.NullInt64
	err := row.Scan(&user_id)
	return user_id, err
}
