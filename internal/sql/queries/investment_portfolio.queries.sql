...
--
-- Returns one row per asset class (stock / bond / alternative) for the given
-- user. Each row's `investments` column is a JSON array of that asset class's
-- holdings, or `[]` when the user has no holdings of that kind.
--
-- Why COALESCE(...) ::jsonb: aggregate functions over an empty rowset return
-- SQL NULL, not an empty array. The Go scan target generated by sqlc is
-- `json.RawMessage`, which does NOT implement sql.Scanner and therefore
-- cannot accept driver.Value(<nil>) — the scan blows up with
-- `unsupported Scan, storing driver.Value type <nil> into type
-- *json.RawMessage`. Wrapping each aggregate in COALESCE turns "no holdings"
-- into a literal `[]` jsonb, which the scanner copies verbatim and
-- json.Unmarshal handles as an empty slice. The InvestmentAnalysis with
-- three empty slices then short-circuits in the streaming handler with a
-- 422 "no investments to analyze" instead of bubbling up as a 500.
--
-- The trailing `::jsonb` is for sqlc's benefit: bare COALESCE(jsonb, jsonb)
-- inside a UNION ALL trips its type inference and falls back to interface{}
-- in the generated row struct. The explicit outer cast pins it back to
-- json.RawMessage at the consumer.
SELECT
